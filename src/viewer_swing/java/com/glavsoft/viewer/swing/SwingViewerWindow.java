...
import main.java.com.glavsoft.core.SettingsChangedEvent;
import main.java.com.glavsoft.rfb.IChangeSettingsListener;
...
import viewer_swing.java.com.glavsoft.viewer.swing.gui.OptionsDialog;
...
public class SwingViewerWindow implements IChangeSettingsListener, WindowFocusListener {
    ...
    private static List<JComponent> kbdButtons;

    
    public static List<JComponent> getKbdButtons() {
        return kbdButtons;
    }
...
//			frame.pack();
    ...
    protected void createButtonsPanel(final ProtocolContext context, Container container) {
        ...
        buttonsBar.createButton("refresh", "Fix keys", new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, false));
                context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, false));
                context.sendMessage(new KeyEventMessage(KeyEvent.VK_SHIFT, false));
                for(JComponent btn : getKbdButtons()){
                    if(btn instanceof JToggleButton){
                        if(((JToggleButton) btn).isSelected()){
                            ((JToggleButton) btn).doClick();
                        }
                    }
                }
        ...
        JToggleButton ctrlButton = buttonsBar.createToggleButton("ctrl", "Ctrl Lock",
                new ItemListener() {
                    ...
                    public void itemStateChanged(ItemEvent e) {
                        if (e.getStateChange() == ItemEvent.SELECTED) {
                            context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, true));
                        } else {
                            context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, false));
                        }
                        setSurfaceToHandleKbdFocus();
        ...
        JToggleButton altButton = buttonsBar.createToggleButton("alt", "Alt Lock",
                new ItemListener() {
                    ...
                    public void itemStateChanged(ItemEvent e) {
                        if (e.getStateChange() == ItemEvent.SELECTED) {
                            //context.sendMessage(new KeyEventMessage(Keymap.K_ALT_LEFT, true));ORIGINAL
                            context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, true));
                        } else {
                           // context.sendMessage(new KeyEventMessage(Keymap.K_ALT_LEFT, false));ORIGINAL
                            context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, false));
                        }
                        setSurfaceToHandleKbdFocus();
                    }
    ...
    private void sendCtrlAltDel(ProtocolContext context) {
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, true));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, true));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_DELETE, true));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_DELETE, false));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, false));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, false));
    }
    ...
    private void sendWinKey(ProtocolContext context) {
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, true));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_ESCAPE, true));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_ESCAPE, false));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, false));
    }
    ...
    public void windowLostFocus(WindowEvent arg0) {
        ...
        refreshKeys(workingProtocol);
    }
    
    private void refreshKeys(ProtocolContext context){
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_CONTROL, false));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_ALT, false));
        context.sendMessage(new KeyEventMessage(KeyEvent.VK_SHIFT, false));
        for(JComponent btn : getKbdButtons()){
        ...
        System.out.println("refreshed keys");
    }
}