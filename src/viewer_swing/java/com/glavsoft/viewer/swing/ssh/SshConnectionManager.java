...
package viewer_swing.java.com.glavsoft.viewer.swing.ssh;
...
import viewer_swing.java.com.glavsoft.viewer.CancelConnectionException;
...
public class SshConnectionManager implements SshKnownHostsManager {
	...
	public int connect(ConnectionParams connectionParams) throws CancelConnectionException {
		...
		try {
			...
			session.connect();
            sync();
			session.setPortForwardingL(0, connectionParams.hostName, connectionParams.getPortNumber());
        } catch (JSchException e) {
            session.disconnect();
